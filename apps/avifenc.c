...
int main(int argc, char * argv[])
{
    ...
    while (argIndex < argc) {
        ...
        if (!strcmp(arg, "-h") || !strcmp(arg, "--help")) {
            ...
            avifEncoderSetCodecSpecificValue(encoder, tempBuffer, value);
            free(tempBuffer);
    ...
    uint32_t unusedIter = 0;
    const char * unusedKey;
    while ((unusedKey = avifEncoderGetNextUnusedCodecSpecificValue(encoder, &unusedIter)) != NULL) {
        fprintf(stderr, "WARNING [-a]: Codec-specific value was unused by the codec: %s\n", unusedKey);
    }
